...
#pragma warning restore CS0542 // 'Program': member names cannot be the same as their enclosing type
        {
            ...
            public class foo1
            {
                ...
                public foo1(int a, float b, string c)
                {
                    this.a = a;
                    this.b = b;
                    this.c = c;
            ...
            class foo2 : foo1
            {
                public foo2(foo1 template): base(template.a, template.b, template.c)
                {
            ...
            */
            /*
                        ...
                        List<string> GroupTotal(_Display display, IMyBlockGroup targetGroup)
            {
                ...
                switch (display.Module.Mode)
                {
                    ...
                    case _ScreenMode.RESOURCE:
                        ...
                        foreach (IMyTerminalBlock nextBlock in targetBlocks)
                        {
                            string definition = nextBlock.BlockDefinition.ToString().Split('_')[1];

                            if (definition == "OxygenTank/")
                                OxyTanks.Add((IMyGasTank)nextBlock);
                            if (definition.Contains("HydrogenTank"))
                                HydroTanks.Add((IMyGasTank)nextBlock);
                            if (definition.Contains("Battery"))
                                Batteries.Add((IMyBatteryBlock)nextBlock);
                            if (definition.Contains("Reactor"))
                                Reactors.Add((IMyReactor)nextBlock);
                            if (definition.Contains("Engine") || definition.Contains("Panel") || definition.Contains("Wind"))
                                PowerProducers.Add((IMyPowerProducer)nextBlock);
                            if (definition.Contains("Generator"))
                                Generators.Add((IMyGasGenerator)nextBlock);
                            if (definition.Contains("Farm"))
                                Farms.Add((IMyOxygenFarm)nextBlock);

                            string info = nextBlock.DetailedInfo;

                            //if (info.Contains("Current Input"))
                            //PowerConsumers.Add(nextBlock);
                        }

                        foreach (IMyGasTank nextTank in OxyTanks)
                        {
                            //nextTank.
                        }
                ...
                return output;
                     ...
                     string NotationBundler(float value, int sigCount)
            {
                string output = string.Empty;

                int decimalplace = -1;                              // Instantiate 1 less than leading digit
                bool negative = false;
                bool zero = true;
                bool decimalFound = false;

                List<char> source = value.ToString().ToList();      // Create a list of all chars in the float
                List<char> sigDigs = new List<char>();              // Empty list to contain final sig-digs

                if (source[0] == '-')                               // Check for negative value
                {
                    source.RemoveAt(0);
                    negative = true;
                }
                ...
                {
                    if (source[i] != '.')                           // Check Decimal found
                    {
                        if (source[i] != '0')                       // Corner-case for zero only number
                            zero = false;

                        if (!decimalFound)                          // Increment/Decrement decimal place based on decimalFound bool and zero corner case
                            decimalplace++;
                        if (decimalFound && zero)
                            decimalplace--;

                        if (sigCount > 0 && !zero)                  // Append char only if not exceeding sigCount and not a leading zero
                        {
                            sigDigs.Add(source[i]);
                            sigCount--;
                        }
                    }
                    else
                ...
                {
                    if (negative)                                       // Re-add the negative
                        output += "-";

                    output += sigDigs[0] + ".";                         // Place leading digit and decimal

                    for (int i = 1; i < sigDigs.Count; i++)             // Append remaing sigDigs
                        output += sigDigs[i];


                    output += "e" + decimalplace;                       // Append scientific notation
                }
            ...
            static char rgb(byte r, byte g, byte b)
            {
                return (char)(0xe100 + (r << 6) + (g << 3) + b);
                        ...
                        void ColorUpdate()
            {
                MonoColors[0] = rgb(0, 7, 0); // Green
                MonoColors[1] = rgb(3, 3, 0); // Yellow
                MonoColors[2] = rgb(7, 0, 0); // Red
                MonoColors[3] = rgb(0, 0, 0); // Black
            }

                        char[] MonoColors = new char[4];

                        string BlockStatusBuilder(_Block Block)
            {
                string output = "*" + Seperator + Block.CustomName + Seperator;

                IMyTerminalBlock targetblock = Block.TermBlock;

                if (targetblock != null)
                {

                    string[] blockDetails = targetblock.DetailedInfo.Split('\n');
                    string crop;

                    float MaxReqInput = 0;
                    float CurrentInput = 0;

                    IMyFunctionalBlock blockFunction = null;
                    IMySlimBlock blockSlim = null;

                    try
                    {
                        blockFunction = (IMyFunctionalBlock)targetblock;
                    }
                    catch
                    {
                        blockFunction = null;
                    ...
                    try
                    {
                        blockSlim = targetblock.CubeGrid.GetCubeBlock(targetblock.Position);
                    }
                    catch
                    {
                        blockSlim = null;
                    ...
                    crop = Array.Find(blockDetails, x => x.Contains("Max Required Input"));
                    if (crop != null)
                    {
                        crop = crop.Split(':')[1];
                        MaxReqInput = float.Parse(crop.Split(' ')[1]);
                        if (crop.Split(' ')[2].Contains("M"))
                            MaxReqInput *= 1000;
                    }

                    crop = Array.Find(blockDetails, x => x.Contains("Current Input"));
                    if (crop != null)
                    {
                        crop = crop.Split(':')[1];
                        CurrentInput = float.Parse(crop.Split(' ')[1]);
                        if (crop.Split(' ')[2].Contains("M"))
                            CurrentInput *= 1000;
                    }
                    ...
                    else
                    {
                        if (CurrentInput == 0)
                            output += MonoColors[2] + "|";
                        if (CurrentInput > 0 && CurrentInput < MaxReqInput)
                            output += MonoColors[1] + "|";
                        if (CurrentInput == MaxReqInput)
                            output += MonoColors[0] + "|";
                    }

                    if (blockSlim != null)
                    {
                        float health = 1 - (blockSlim.CurrentDamage / blockSlim.BuildIntegrity);
                        if (health == 1)
                            output += MonoColors[0] + "|";
                    ...
                    if (targetblock.IsBeingHacked)
                        output += MonoColors[2] + "|";
                    else
                        output += MonoColors[0] + "|";
                }
                else
                    output = "!" + Seperator + "Block Missing! Refresh Block Detection!";

                return output;
            }

                        void ScreenOut(string target, string log, bool append = false)
            {
                foreach (IMyTextPanel nextPanel in PanelBlocks)
            ...
            */
            ...
            #region TODO LIST
            /* minifying? 
             * All Target Variants (Grid-groups, grid-blocks, all-groups, all-blocks)
             * Power Management (is it done?)
             * Connector support
            ...
            public readonly char[] EchoLoop = new char[]
{
            ...
            /// Helpers
            bool ContainsCIS(string source, string target)// Case In-Sensitive
            {
            ...
            string[] ItemTypeCrop(MyInventoryItem targetItem)
            {
            ...
            string ProductionIdCrop(string blueprintId)
            {
            ...
            /// Inventory
            IMyInventory PullInventory(_Inventory inventory, bool input = true)
            {
            ...
            bool FilterCompare(_FilterProfile profile, MyInventoryItem item, out MyFixedPoint target, bool dirIn = true)
            {
            ...
            void InventoryEmpty(_Inventory inventory)
            {
                ...
                foreach (MyInventoryItem nextItem in sourceItems)
                {
                    ...
                    if (FilterCompare(inventory.FilterProfile, nextItem, out target, false))
                        EmptyToCandidate(sourceInventory, nextItem);
                }
            }
            ...
            void EmptyToCandidate(IMyInventory sourceInventory, MyInventoryItem nextItem)
            {
                ...
                for (int i = InvSearchIndex; i < InvSearchIndex + InvSearchCap && i < Inventories.Count(); i++)
                {
                    ...
                    IMyInventory targetInventory = PullInventory(Inventories[i]);
            ...
            void ProductionTally(_Production prod)
            {
                ...
                targets = allItems.FindAll(x => modDef == x.Type.ToString().Split('/')[1]);
            ...
            void ProducerClean(_Producer producer)
            {
                if (!producer.CLEAN)
                    return;
            ...
            void BlockListUpdate()
            {
                ...
                if (bProductionRunning)
                {
                    ...
                    {
                        ProducerClean(Producers[CleanQueIndex]);
